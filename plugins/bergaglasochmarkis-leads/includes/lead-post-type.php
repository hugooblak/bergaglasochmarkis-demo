...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'bergaglasochmarkis-leads' ),
					'singular_name' => __( 'Lead', 'bergaglasochmarkis-leads' ),
					'edit_item'     => __( 'Förfrågan', 'bergaglasochmarkis-leads' ),
					'search_items'  => __( 'Search leads', 'bergaglasochmarkis-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'bergaglasochmarkis-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'bergaglasochmarkis-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'bergaglasochmarkis-leads' ),
			'npl_phone'    => __( 'Phone', 'bergaglasochmarkis-leads' ),
			'npl_service'  => __( 'Needs', 'bergaglasochmarkis-leads' ),
			'npl_timeline' => __( 'When', 'bergaglasochmarkis-leads' ),
			'npl_zip'      => __( 'ZIP', 'bergaglasochmarkis-leads' ),
			'npl_source'   => __( 'Source', 'bergaglasochmarkis-leads' ),
			'npl_status'   => __( 'Status', 'bergaglasochmarkis-leads' ),
			'date'         => __( 'Received', 'bergaglasochmarkis-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'bergaglasochmarkis-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'bergaglasochmarkis-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'bergaglasochmarkis-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'bergaglasochmarkis-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'bergaglasochmarkis-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'bergaglasochmarkis-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'bergaglasochmarkis-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'bergaglasochmarkis-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'bergaglasochmarkis-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'bergaglasochmarkis-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'bergaglasochmarkis-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'bergaglasochmarkis-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
