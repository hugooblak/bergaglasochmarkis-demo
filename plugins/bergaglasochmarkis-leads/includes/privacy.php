...
add_filter(
	...
	function ( $exporters ) {
		$exporters['bergaglasochmarkis-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'bergaglasochmarkis-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'bergaglasochmarkis-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'bergaglasochmarkis-leads',
						'group_label' => __( 'Quote requests', 'bergaglasochmarkis-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['bergaglasochmarkis-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'bergaglasochmarkis-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
