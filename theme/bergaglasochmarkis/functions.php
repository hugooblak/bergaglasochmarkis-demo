<?php
/**
 * Berga glas och markis theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Berga glas och markis Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through be_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'bergaglasochmarkis', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'bergaglasochmarkis' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'bergaglasochmarkis' ) ) );

		register_block_pattern_category( 'bergaglasochmarkis-sections', array( 'label' => __( 'Berga glas och markis sections', 'bergaglasochmarkis' ) ) );
		register_block_pattern_category( 'bergaglasochmarkis-pages', array( 'label' => __( 'Berga glas och markis page layouts', 'bergaglasochmarkis' ) ) );
	}
);
