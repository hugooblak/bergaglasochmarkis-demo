 ...
 * or photo. They call be_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Berga glas och markis.
 *
 ...
 * @return string
 */
function be_sample_note() {
	return be_lead( 'ar_riktig' ) ? '' : ' <span class="be-sample">(platshållare)</span>';
}
 ...
 * @return string File name for be_lead_img().
 */
function be_lead_bild( $i ) {
	$bilder = (array) be_lead( 'bilder', array() );
 ...
 * @return string
 */
function be_ring_label() {
	$namn = be_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, be_lead( 'telefon' ) ) : 'Ring ' . be_lead( 'telefon' );
}
